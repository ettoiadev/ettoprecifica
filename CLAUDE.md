...
- Internamente: lê o CEP de origem de uma tabela (não hardcode) — assumido `select cep_origem_loja from calc_deslocamento where ativo=true limit 1` (**não confirmado**, pedimos ao Devin validar se essa tabela/colunas existem de fato antes de codar), calcula distância via API de rota (sugestão: OpenRouteService, gratuita) e chama `calc_deslocamento(p_distancia_km, p_tempo_instalacao_horas, p_qtd_funcionarios)` — assinatura também **não confirmada** pelo lado do app, Devin deve validar via `pg_get_function_arguments` antes de codar (gotcha 1b).
