...
Quando o Devin confirmar o slug/contrato real, o trabalho do lado do app é: trocar o input manual de R$ por um campo de CEP + campo de tempo estimado de instalação nas 12 calculadoras, chamar essa função pra pré-preencher `custoDeslocamento` (vendedor ainda revisa/ajusta antes de confirmar), mantendo o checkbox "Incluir deslocamento" como opt-in (off por padrão). **Reconfirmar o contrato real via SQL antes de integrar** — não assumir que o brief foi implementado exatamente como pedido.
