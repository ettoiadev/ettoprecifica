...
**Pendência aberta**: a API de rota (CEP→CEP) que alimentaria `calc_deslocamento` com a distância real é trabalho do Devin, ainda não integrada — até lá o valor de deslocamento continua 100% manual (o vendedor digita). Em 27/07/26 o usuário testou manualmente o fluxo completo (loja CEP 12321-150 → cliente CEP 12301-330, ambos Jacareí: ~6,05km → `calc_deslocamento` retornou R$10,18) confirmando que a fórmula funciona; só falta automatizar CEP→km. Foi enviado ao Devin um brief técnico pedindo uma função nova (nome/local a definir pelo Devin — ver hospedagem abaixo) com o seguinte contrato esperado:
- **Request**: `{ cep_destino, tempo_instalacao_horas?, qtd_funcionarios? }`.
- Internamente: lê o CEP de origem de uma tabela (não hardcode) — `select cep_origem_loja from calc_deslocamento where ativo=true limit 1`. **Confirmado pelo usuário em 27/07/26**: tabela `calc_deslocamento` (colunas `cep_origem_loja`, `ativo`) coexiste com a função `calc_deslocamento(...)` de mesmo nome — mesmo padrão nome-tabela/nome-função já usado em `calc_lona`, `calc_placa_acm`, `calc_vidro`, `calc_dtf` etc. (Postgres separa `pg_class`/`pg_proc`, sem ambiguidade). Calcula distância via **OpenRouteService** (perfil `driving-car`; usuário já gerou a key, plano gratuito 2.000 req/dia) e chama `calc_deslocamento(p_distancia_km, p_tempo_instalacao_horas, p_qtd_funcionarios)` — essa assinatura ainda não foi verificada por SQL do lado do app; Devin deve confirmar via `pg_get_function_arguments` antes de codar (gotcha 1b), mas a leitura da tabela de origem já está validada.
- **Response** esperado: `{ distancia_km, trecho, custo_km, custo_mao_obra_excedente, custo_alimentacao, custo_deslocamento }` (detalhado, não só o total, pro vendedor entender de onde veio o valor).
- **Hospedagem em aberto**: o app roda em **Vercel** e **Railway** além do Supabase (banco) — decisão de onde a chamada à OpenRouteService vai morar (Supabase Edge Function, API route na Vercel, ou serviço na Railway) foi delegada ao Devin. A key da OpenRouteService será cadastrada como env var/secret na plataforma escolhida, nunca hardcoded — o usuário cadastra depois que o Devin confirmar onde.

Quando o Devin confirmar onde implementou e o slug/URL/contrato real, o trabalho do lado do app é: trocar o input manual de R$ por um campo de CEP + campo de tempo estimado de instalação nas 12 calculadoras, chamar essa função pra pré-preencher `custoDeslocamento` (vendedor ainda revisa/ajusta antes de confirmar), mantendo o checkbox "Incluir deslocamento" como opt-in (off por padrão). **Reconfirmar o contrato real antes de integrar** — não assumir que o brief foi implementado exatamente como pedido.
